...
if($row['total'] == 0) {
    ...
    // Cek kecamatan
    $cek_kec = pg_query($conn, "SELECT COUNT(*) as total FROM kecamatan");
    $kec = pg_fetch_assoc($cek_kec);
    ...
    // Cek kelurahan
    $cek_kel = pg_query($conn, "SELECT COUNT(*) as total FROM kelurahan");
    $kel = pg_fetch_assoc($cek_kel);
